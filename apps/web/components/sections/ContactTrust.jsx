        ...
        <RevealGroup className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {REASONS.map((reason) => (
            <RevealItem key={reason.title} className="h-full">
              <article className="h-full rounded-2xl border border-border bg-card p-8 transition-colors duration-300 hover:border-primary">
                {reason.icon}

                <h3 className="mt-6 font-sans text-xs uppercase tracking-[0.25em] text-primary">
                  {reason.title}
                </h3>
